...
2. Add the DNAT rules on the proxmox host:
   ...
   ```bash
   ssh root@kvm.example.net -p 6022 \
...
4. Update PowerDNS so internal clients bypass the tunnel:
   ...
   public IP `vmbr0`, aliased to `kvm.example.net` for inbound HTTPS.)
