...
|---|---|---|---|---|---|
| SC-001 | Clean-room end-to-end bring-up completes in <=60 minutes | `time (make build-image && python scripts/apply_tofu.py cicd && python scripts/apply_tofu.py apps && python tools/bootstrap_cluster.py --cluster cicd && python tools/bootstrap_cluster.py --cluster apps)` | total wall-clock <= 60 min | SS1+SS2+SS3 | deferred (no live PVE host in CI) |
...
| SC-007 | Phase 1 produces an Ubuntu+k3s template at VMID 900 with `image-id.txt = "900\n"` on the live host, AND the 4 cluster VMs (111-114) are cloned from it with a working qemu-guest-agent | `ssh root@$PVE_HOST 'grep -E "^template:" /etc/pve/qemu-server/900.conf && for vmid in 900 111 112 113 114; do qm agent $vmid ping; done' && cat build/image-id.txt` | `template: 1`, all 5 `qm agent` pings return 0, `image-id.txt` is `900` | SS0+SS1+SS2 | **pass** (verified 2026-07-07 on BigBertha PVE 9.2.3 via the canonical Proxmox+Ubuntu recipe: virt-customize + native cloud-init drive) |
...
- **pytest**: `pytest tools/tests/` reports **92 passed** as of 2026-07-07
  (43 baseline WP00-WP06 + 8 WP07 acceptance + 11 Skill
  Step 0a/0b/0c/0d pins + 7 Step 1b Phase-1 pins + **23 v2-cleanup
  pins** that cover the canonical Proxmox+Ubuntu recipe: virt-customize,
  native cloud-init drive, VMID 900, SDN DHCP, etc.).
- **tofu tests**: `tofu test` passes for the four tofu modules
  (12 cluster-module + 2 cicd-instance + 5 apps-instance + 6 tokens,
  with the spec-T005 12-priv set extended to 20 privs to cover
  Phase 1 + Phase 2 VM lifecycle and SDN writes)
  = 25 passed / 0 failed).